...
- `main` is clean and fully in sync with `origin/main` (0 ahead/behind), at `b56a2eb` — the Router-`basename` + GitHub Pages 404-fallback fix (see below). `npm run deploy` republished `build/` (now including `build/404.html`) to GitHub Pages after the push.
